...
class ProductionReadinessValidatorTest {
    ...
    fun accepts_hardened_production_profile() {
        ...
        val config = baseConfig(
            ...
            security = baseConfig().security.copy(
                mtls = baseConfig().security.mtls.copy(
                    grpc = AppConfig.SecurityConfig.MtlsConfig.GrpcMtlsConfig(
                        enabled = true,
    ...
    private fun baseConfig(
        ...
        database: AppConfig.DatabaseConfig = AppConfig.DatabaseConfig(
        ...
        security: AppConfig.SecurityConfig = AppConfig.SecurityConfig(
            jwt = AppConfig.SecurityConfig.JwtConfig(
                realm = "NeoGenesis",
            ...
            mtls = AppConfig.SecurityConfig.MtlsConfig(
                grpc = AppConfig.SecurityConfig.MtlsConfig.GrpcMtlsConfig(
                    enabled = false,
                    requireClientAuth = true,
                    certChainPath = null,
                ...
                mqtt = AppConfig.SecurityConfig.MtlsConfig.MqttMtlsConfig(
                    enabled = false,
                    keyStorePath = null,
                ...
                httpProxyValidation = AppConfig.SecurityConfig.MtlsConfig.HttpProxyValidationConfig(
                    enabled = false,
        ...
        encryption: AppConfig.EncryptionConfig = AppConfig.EncryptionConfig(
            enabled = false,
            phiKeyRef = "env:NEOGENESIS_PHI_KEY_B64",
            piiKeyRef = "env:NEOGENESIS_PII_KEY_B64"
        ),
        serverless: AppConfig.ServerlessConfig = AppConfig.ServerlessConfig(
            outboxBatchSize = 100,
            ...
            baseBackoffMs = 500,
            maxBackoffMs = 30_000,
            awsSqs = AppConfig.ServerlessConfig.AwsSqsConfig(
                queueUrl = null,
                region = "eu-west-1",
                endpointOverride = null
            ),
            awsEventBridge = AppConfig.ServerlessConfig.AwsEventBridgeConfig(
                eventBusName = "neogenesis-bus",
                sourceNamespace = "com.neogenesis.core",
                region = "eu-west-1",
                endpointOverride = null
            ),
            gcpPubSub = AppConfig.ServerlessConfig.GcpPubSubConfig(
                projectId = null,
                topicId = null,
                emulatorHost = null
            )
        ...
        return AppConfig(
            ...
            database = database,
            mqtt = null,
            clinical = AppConfig.ClinicalConfig(
                dicomweb = AppConfig.ClinicalConfig.DicomWebConfig(
                    enabled = false,
                    baseUrl = null,
                    bearerToken = null,
                    timeoutMs = 7000
                ),
                dimse = AppConfig.ClinicalConfig.DimseConfig(
                    enabled = false,
                    findScuPath = null,
                    moveScuPath = null,
                    getScuPath = null,
                ...
                hl7Mllp = AppConfig.ClinicalConfig.Hl7MllpConfig(
                    enabled = false,
                    host = "0.0.0.0",
                    port = 2575,
                ...
                validation = AppConfig.ClinicalConfig.ValidationConfig(
                    enforceStrict = true,
            ...
            security = security,
            secrets = AppConfig.SecretsConfig(
                vaultEnabled = false,
                vaultAddress = null,
                vaultToken = null,
                vaultMount = "secret",
            ...
            resilience = AppConfig.ResilienceConfig(
                enabled = true,
            ...
            control = AppConfig.ControlConfig(
                latencyBudgetMs = 50
            ),
