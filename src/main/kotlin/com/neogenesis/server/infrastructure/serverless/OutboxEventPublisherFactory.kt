...
object OutboxEventPublisherFactory {
    fun create(config: AppConfig.ServerlessConfig): OutboxEventPublisher {
        return when (config.provider.lowercase()) {
            "aws_sqs", "sqs" -> {
                val queueUrl = requireNotNull(config.awsSqs.queueUrl) {
            ...
            "aws_eventbridge", "eventbridge" -> {
                val eventBusName = requireNotNull(config.awsEventBridge.eventBusName) {
            ...
            "gcp_pubsub", "pubsub" -> {
                val projectId = requireNotNull(config.gcpPubSub.projectId) {
                    "neogenesis.serverless.gcpPubSub.projectId is required when provider=gcp_pubsub"
                }
                val topicId = requireNotNull(config.gcpPubSub.topicId) {
