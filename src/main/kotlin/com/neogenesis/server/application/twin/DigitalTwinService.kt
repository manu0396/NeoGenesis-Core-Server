...
class DigitalTwinService(
    ...
    fun updateFromTelemetry(
        ...
        val temperaturePenalty = (abs(telemetry.nozzleTempCelsius - 37.0f) / 20.0f).coerceIn(0.0f, 1.0f)
        val pressurePenalty = (abs(telemetry.extrusionPressureKPa - 110.0f) / 180.0f).coerceIn(0.0f, 1.0f)
        val morphologyPenalty = telemetry.morphologicalDefectProbability.coerceIn(0.0f, 1.0f)
        val phPenalty = (abs(telemetry.bioInkPh - 7.35f) / 1.2f).coerceIn(0.0f, 1.0f)
        val nirPenalty = ((telemetry.nirIiTempCelsius - 37.0f).coerceAtLeast(0.0f) / 5.0f).coerceIn(0.0f, 1.0f)
        val viscosityPenalty = (abs(telemetry.bioInkViscosityIndex - 0.8f) / 0.8f).coerceIn(0.0f, 1.0f)
        val shearPenalty = ((simulatedShearStressKPa ?: 0.0f) / 450.0f).coerceIn(0.0f, 1.0f)
        val emergencyPenalty = if (command.actionType == ControlActionType.EMERGENCY_HALT) 0.25f else 0.0f
        ...
        val predictedViability = ((physicsViability + viability) / 2.0f - (collapseRisk * 0.16f)).coerceIn(0.0f, 1.0f)
        val confidence = (
            1.0f - ((temperaturePenalty + pressurePenalty + morphologyPenalty + phPenalty + nirPenalty + viscosityPenalty + shearPenalty) / 7.0f)
            ).coerceIn(0.2f, 0.99f)

        val state = DigitalTwinState(
